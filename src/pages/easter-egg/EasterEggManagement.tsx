...
function EasterEggCardSkeleton() {
  return (
...
function EasterEggMobileSkeleton() {
  return (
...
export function EasterEggManagement() {
  ...
  const handleFeatureStatusToggle = async (newValue: boolean) => {
    try {
      ...
      // Eierliste neu laden nach Toggle
  ...
  const handleStartDateSave = async () => {
    try {
      ...
      showSuccessMessage(toast, {
        ...
        description: `Das Startdatum wurde auf ${formatDate(startDateInput)} gesetzt.`,
      });
    } catch (error) {
...
const EasterEggCard: React.FC<EasterEggCardProps> = ({ egg, onDelete }) => {
  const navigate = useNavigate();
    ...
    <motion.div whileHover={{ scale: 1.02 }} transition={defaultTransition}>
      <Card className={cn(glassCardHover, 'flex flex-col gap-0 !py-0 !px-0 overflow-hidden')}>
        ...
        <CardFooter className="flex justify-between items-center !px-4 !pt-2 !pb-4 gap-0">
          <div className="text-xs text-muted-foreground">{formatDate(egg.createdAt)}</div>
          <div className="flex gap-2">
            <AnimatedButton
              variant="outline"
              size="sm"
...
const EasterEggCardMobile: React.FC<EasterEggCardMobileProps> = ({ egg, onDelete, navigate }) => {
  return (
    <Card className={cn(glassCardHover, 'p-4')}>
      <div className="flex flex-col gap-2">
        ...
        <div className="text-xs text-muted-foreground mt-1">
          Erstellt am {formatDate(egg.createdAt)}
        </div>
