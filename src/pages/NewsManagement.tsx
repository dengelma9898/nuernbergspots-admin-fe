      ...
      <CardContent className="p-4 md:p-6">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <span className="font-semibold text-sm text-white">{item.authorName || 'Unbekannt'}</span>
          </div>
        ...
        {/* Content */}
        {item.type === 'text' && (
          <div className="text-base leading-relaxed py-2 text-white">{(item as TextNewsItem).content}</div>
        )}
        ...
        {item.type === 'image' && (
          <div>
            ...
            <div className="text-base leading-relaxed py-1 text-white">{(item as ImageNewsItem).content}</div>
          </div>
        )}
          ...
          <div className="py-2">
            ...
            <div className="flex flex-col gap-2">
              {(item as PollNewsItem).options.map(opt => (
                ...
                >
                  <span>{opt.text}</span>
                  <Badge className="bg-white/20 text-white border-white/30">{opt.voters.length}</Badge>
                </Button>
              ))}
            </div>
            {item.expiresAt && (
            ...
            <div>
              ...
              <div className="flex gap-3 flex-wrap">
                {imagePreviews.map((url, idx) => (
                  <div key={idx} className="relative w-20 h-20 border border-white/30 rounded-xl overflow-hidden ring-2 ring-white/20">
        ...
        <DialogContent className="backdrop-blur-3xl bg-white/10 border-white/20 text-white max-w-lg">
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="poll-question" className="text-white/90">Frage</Label>
              <Input
                id="poll-question"
                value={pollQuestion}
            ...
            <div className="flex items-center space-x-3 p-3 backdrop-blur-2xl bg-white/5 rounded-xl border border-white/10">
              <Switch
                ...
                disabled={pollSending}
              />
              <Label htmlFor="multiple-answers" className="text-white/90">Mehrfachauswahl erlauben</Label>
            </div>

            <div className="space-y-2">
              <Label htmlFor="poll-expires" className="text-white/90">Ablaufdatum (optional)</Label>
              <Input
                id="poll-expires"
                type="datetime-local"
                value={pollExpiresAt}
