...
describe('Event Service', () => {
  ...
  describe('scrapeEventsWithLlm', () => {
    it('should scrape events with LLM from URL', async () => {
      const mockEvents = [{ id: '1', title: 'Scraped Event' }];
      ...
      const result = await eventService.scrapeEventsWithLlm('https://eventfinder.de/nuernberg', true);

      expect(mockApi.post).toHaveBeenCalledWith('/events/scrape/llm', {
    ...
    it('should scrape events without fallback', async () => {
      const mockEvents = [{ id: '1', title: 'Scraped Event' }];
      mockApi.post.mockResolvedValue({
        data: {
          events: mockEvents,
      ...
      const result = await eventService.scrapeEventsWithLlm('https://eventfinder.de/nuernberg', false);
    ...
    it('should use default fallback value (true)', async () => {
      const mockEvents = [{ id: '1', title: 'Scraped Event' }];
      mockApi.post.mockResolvedValue({
        data: {
          events: mockEvents,
      ...
      const result = await eventService.scrapeEventsWithLlm('https://eventfinder.de/nuernberg');
    ...
    it('should handle API errors', async () => {
      ...
      await expect(
        eventService.scrapeEventsWithLlm('https://eventfinder.de/nuernberg', true)
  ...
  describe('scrapeEventsFromEventFinder', () => {
    it('should scrape events with parameters', async () => {
      const mockEvents = [{ id: '1', title: 'Scraped Event' }];
      mockApi.get.mockResolvedValue({ data: mockEvents });
      ...
      const result = await eventService.scrapeEventsFromEventFinder(params);

      expect(mockApi.get).toHaveBeenCalledWith(
        '/events/scrape?type=concert&category=music&startDate=2024-01-01&endDate=2024-01-31&maxResults=50'
      );
      expect(result).toEqual(mockEvents);
    });
  });
