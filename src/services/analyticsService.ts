...
export function useAnalyticsService() {
  ...
  const calculatePeakTimes = (scans: CustomerScan[]) => {
    ...
    const peakDay = Object.entries(dayCount).sort(([, a], [, b]) => b - a)[0]?.[0] || '';
  ...
  const calculateBusinessAnalytics = (business: BusinessCustomerScans): BusinessAnalytics => {
    ...
    const previousWeekScans = calculateDateRangeScans(customerScans, DAYS_PER_WEEK * 2).slice(
      0,
      -DAYS_PER_WEEK
    );
    const previousMonthScans = calculateDateRangeScans(customerScans, DAYS_PER_MONTH * 2).slice(
      0,
      -DAYS_PER_MONTH
    );
    ...
    const averageNumberOfPeople =
      peopleCount.length > 0 ? peopleCount.reduce((a, b) => a + b, 0) / peopleCount.length : 0;
  ...
  const calculateDashboardAnalytics = (businesses: BusinessCustomerScans[]): DashboardAnalytics => {
    ...
    const monthlyCustomers = new Set(
      calculateDateRangeScans(allScans, DAYS_PER_MONTH).map(scan => scan.customerId)
    ).size;
    ...
    const returningCustomers = Object.values(customerVisits).filter(visits => visits > 1).length;
    const returningRate =
      uniqueCustomerIds.size > 0 ? (returningCustomers / uniqueCustomerIds.size) * 100 : 0;
    ...
    return {
      ...
      topBusinesses: [...businessAnalytics].sort((a, b) => b.totalScans - a.totalScans).slice(0, 5),
      ...
      revenueData: {
        ...
        averagePerScan,
        projectedMonthly:
          (totalRevenue / allScans.length) * (totalScans / (allScans.length / DAYS_PER_MONTH)),
      },
      customerData: {
